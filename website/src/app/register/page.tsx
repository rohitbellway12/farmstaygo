...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiFetch<RegisterResponse>("/auth/register-user", {
        method: "POST",
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-brand-700 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 xl:px-20">
          ...
          <h1 className="text-4xl font-bold leading-[1.1] tracking-tight xl:text-5xl">
            Start your journey with FarmStayGo.
          </h1>
          <p className="mt-6 max-w-md text-base leading-7 text-white/75 xl:text-lg">
            Create an account to book verified farmhouses, villas, and unique nature stays across India.
          </p>
        </div>
      </div>
      ...
      <div className="flex w-full items-center justify-center bg-ink-50 px-5 py-12 lg:w-1/2">
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl border border-ink-200 bg-white p-6 shadow-[0_4px_18px_rgba(22,52,35,0.06)] sm:p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">Create your account</h2>
              <p className="mt-2 text-sm text-ink-500">Join FarmStayGo and start booking unique stays.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="firstName" className="mb-2 block text-sm font-semibold text-ink-700">
                ...
                <div>
                  <label htmlFor="lastName" className="mb-2 block text-sm font-semibold text-ink-700">
                    Last name
                  </label>
                  <input
                    id="lastName"
              ...
              <div>
                <label htmlFor="mobile" className="mb-2 block text-sm font-semibold text-ink-700">
                  Mobile number
                </label>
                <input
                  id="mobile"
          ...
          <p className="mt-6 text-center text-xs text-ink-400">
            By creating an account, you agree to FarmStayGo&apos;s terms and privacy policy.
          </p>
        </div>
      </div>
    </div>
